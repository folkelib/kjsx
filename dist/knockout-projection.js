...
var StateItem = /** @class */ (function () {
    function StateItem(inputItem, stateArrayIndex, initialOutputArrayIndex, mappingOptions, arrayOfState, outputObservableArray) {
        this.inputItem = inputItem;
        this.stateArrayIndex = stateArrayIndex;
...
function insertOutputItem(diffEntry, movedStateItems, stateArrayIndex, outputArrayIndex, mappingOptions, arrayOfState, outputObservableArray, outputArray) {
    ...
    var moved = diffEntry.moved;
    var stateItem = typeof moved === "number" ? movedStateItems[moved] :
...
function respondToArrayStructuralChanges(inputObservableArray, arrayOfState, outputArray, outputObservableArray, mappingOptions) {
    return inputObservableArray.subscribe(function (diff) {
        ...
        var diffIndex = 0;
        var diffEntry = diff[0];
        var editOffset = 0; // A running total of (num(items added) - num(items deleted)) not accounting for filtering
        var outputArrayIndex = diffEntry && getFirstModifiedOutputIndex(diffEntry, arrayOfState, outputArray);
...
// Mapping
function map(inputObservableArray, mappingOptions) {
    var arrayOfState = [];
    var outputArray = [];
    ...
    for (var i = 0; i < originalInputArrayContents.length; i++) {
        ...
        var stateItem = new StateItem(inputItem, i, outputArray.length, mappingOptions, arrayOfState, outputObservableArray);
    ...
    var inputArraySubscription = respondToArrayStructuralChanges(inputObservableArray, arrayOfState, outputArray, outputObservableArray, mappingOptions);
    ...
    var originalDispose = returnValue.dispose;
...
function filter(array, predicate) {
    return map(array, { mapping: function (item) {
